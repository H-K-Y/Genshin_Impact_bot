...
class Gacha(object):
    ...
    def get_png_path(name):
        ...
        character_icon_path = ASSETS / "角色图鉴" / f"{name}.png"
        weapon_icon_path = ASSETS / "武器图鉴" / f"{name}.png"

        if character_icon_path.exists():
            return str(character_icon_path)

        if weapon_icon_path.exists():
            return str(weapon_icon_path)

        return str(ASSETS / "default.png")
