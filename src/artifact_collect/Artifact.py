...
class Artifact(object):
    ...
    def get_main_value(self):
        ...
        if self.level == 20:
            ...
            return PROPERTY_LIST["main"][self.main]["initial_value"] + self.level * PROPERTY_LIST["main"][self.main][
                "growth_value"]
    ...
    def strengthen(self):
        ...
        if self.level % 4 == 0:
            if len(self.initial_secondary) + len(self.strengthen_secondary_list) < 4:
                ...
                secondary_value = self.get_random_secondary_value(secondary)
                strengthen_type = "add"
                self.strengthen_secondary_list.append(
                    {"type": strengthen_type, "property": secondary, "value": secondary_value})
        ...
        return {"level": self.level, "strengthen_type": strengthen_type, "secondary": secondary,
    ...
    def get_artifact_image(self, number):
        ...
        draw.text((25, 60), ARTIFACT_PROPERTY[self.artifact_type]['name'], fill="#ffffffff",
                  font=ImageFont.truetype(ttf_path, size=20))
        draw.text((25, 130), PROPERTY_LIST['main'][self.main]['txt'], fill="#bfafa8",
                  font=ImageFont.truetype(ttf_path, size=20))
        draw.text((25, 153), self.number_to_str(main_property_value), fill="#ffffffff",
                  font=ImageFont.truetype(ttf_path, size=32))
